...
| [Codex 通知集成协议](./codex-notification-integration.md) | 原生 notify、PermissionRequest hook、wrapper 日志补缺和错误矩阵 |
...
- 修改 Codex hook、事件字段或通知进程前，必须阅读 [Codex 通知集成协议](./codex-notification-integration.md)。
- 不读取、提交或在输出中展示真实 `.env` 内容；配置字段以 `.env.example` 为准。
- 先检查 `git status --short`，保留用户已有的未提交改动。

## 质量检查

- 执行 `python3 -m unittest test_codex_wrapper.py test_notify.py test_codex_hook.py`。
- 执行 `python3 -m py_compile codex-wrapper.py codex-hook.py notify.py test_codex_wrapper.py test_codex_hook.py test_notify.py`。
